...
class OptionParser(metaclass=SingletonMeta):
	...
	def parse(self):
		...
		if "batch" in self.values:
			BATCH_SIZE = self.values.batch
		if "max-palette-size" in self.values:
			MAX_PALETTE_SIZE = self.values["max-palette-size"]
		if "log-folder" in self.values:
			TEMP_FOLDER = self.values["log-folder"]
